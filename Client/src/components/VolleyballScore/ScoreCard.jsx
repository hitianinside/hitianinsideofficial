...
const ScoreCard = ({ score, isLive }) => {
  // Determine who won/is winning to dim the losing team (only after match is finished)
  const t1Score = parseInt(score.team1_score) || 0;
  const t2Score = parseInt(score.team2_score) || 0;
  const t1Highlight = isLive ? true : t1Score >= t2Score;
  const t2Highlight = isLive ? true : t2Score >= t1Score;
  ...
  return (
    <motion.div
      variants={cardVars}
      layout
      className={`relative w-full max-w-[440px] bg-gradient-to-br ${isLive ? 'from-white/10 to-red-900/20' : 'from-white/10 to-gray-900/40'
    ...
    >
      ...
      {/* Match Header */}
      <h2 className={`text-center text-xs md:text-sm font-black uppercase tracking-widest mb-4 md:mb-6 ${isLive ? 'text-yellow-400' : 'text-green-400'
        }`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-4 md:gap-2 mb-4 md:mb-6">
        ...
        <div className="hidden md:flex flex-col items-center justify-center pt-2">
          <div className="text-4xl md:text-5xl font-black tracking-tighter text-white tabular-nums">
            {score.team1_score}<span className="text-white/30 mx-2">:</span>{score.team2_score}
          </div>
      ...
      {/* Footer Status */}
      <div className={`flex items-center justify-center gap-2 pt-3 md:pt-4 border-t border-white/5 ${isLive ? 'text-red-400' : 'text-gray-400'
        }`}>
        {isLive ? (
          <>
